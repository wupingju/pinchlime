...
而 Netlify 最棒的地方在於，他有自動化部署的功能。因為他串接的是 Github 的倉庫，所以每當我的 Github 程式碼更新時，Netlify 就會自動把新的程式碼重新部署到網路上，因此我也不需要一直跑來 Netlify 手動「更新」，而只要專新處理好我要編輯的文字，然後把它們「push」到 Github 裡面就好。
